...
This repository include the groundtruth, manually annotated, the masks produced by the algorithms when fire was found, and some patches from the original image. Those files can found under the path `dataset/groundtruth` in zip files. The script `src/utils/unzip_patches` will unzip this files in separed folders. The folder `manual_annotation` contains the masks generated manually, the folder `masks` holds the masks generated by the literature algorithms, and the folder `patches` contains the images patches with 10 channels.

**Atention:** the zip files under `dataset/groundtruth` are versionaded with Git Large File Storage (LFS), you may need to redownload the files [directly from the repository](https://github.com/pereira-gha/activefire/tree/main/dataset/groundtruth).

The code inside the folder `src/groundtruth` gives you a way to compare both the masks of the algorithms and the output produced by CNN against the Groundtruth. To compare the algorithms (Schroeder, Murphy and GOLI) against the groundtruth run:
