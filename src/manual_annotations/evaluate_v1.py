...
if __name__ == '__main__':
    ...
    for algorithm in MASKS_ALGORITHMS:
        ...
        txts_mask_path = sorted(glob.glob(os.path.join(OUTPUT_DIR, algorithm, 'arrays', 'grd_*.txt')))
        txts_pred_path = sorted(glob.glob(os.path.join(OUTPUT_DIR, algorithm, 'arrays', 'det_*.txt')))
        ...
        for txt_mask_path, txt_pred_path in zip(txts_mask_path, txts_pred_path):
            ...
            try:
                ...
                if txt_mask_path.replace('grd', 'det') != txt_pred_path:
